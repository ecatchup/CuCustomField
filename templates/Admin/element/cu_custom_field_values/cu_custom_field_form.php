				...
				<tr>
					...
					<td class="col-input bca-form-table__input">
						...
						<?php if($definition['CuCustomFieldDefinition']['field_type'] === 'loop'): ?>
							...
							<?php if(!empty($this->getRequest()->getData('CuCustomFieldValue')[$definition['CuCustomFieldDefinition']['field_name']]) &&
									is_array($this->getRequest()->getData('CuCustomFieldValue')[$definition['CuCustomFieldDefinition']['field_name']])): ?>
								<?php foreach($this->getRequest()->getData('CuCustomFieldValue')[$definition['CuCustomFieldDefinition']['field_name']] as $key => $value): ?>
	...
	<ul>
		<li>利用可能なフィールドがありません。不要な場合は
			<?php $this->BcBaser->link('カスタムフィールド設定',
				['plugin' => 'cu_custom_field', 'controller' => 'cu_custom_field_configs', 'action' => 'edit', $this->getRequest()->getData('CuCustomFieldConfig.id')],
				[],

 ...
 * @license          MIT LICENSE
 */
namespace CuCfFile\View\Helper;
    ...
    public function __construct(\Cake\View\View $View, $settings = [])
    {
        ...
        if(empty($valueModel->getBehavior('CuCfFile')->BcFileUploader->settings['saveDir'])) {
            if(BcUtil::isAdminSystem()) {
                // 管理画面ではBcBlogプラグイン内でのみset
                if ($View->getPlugin() == 'BcBlog') {
                   $blogContent = $View->get('blogContent');
                   $blogContentId = isset($blogContent['id']) ? $blogContent['id'] : null;
               }
            } else {
                // 公開側ではpostがある場合のみset
                $post = $View->get('post');
                $blogContentId = !empty($post) ? $post->blog_content_id : false;
            }
...
// '<span class="bca-file"><input type="file" name="CuCustomFieldValue[article][__loop-src__][article_image]" class="bca-file__input" deleteSpan="bca-file__delete" deleteCheckbox="bca-file__delete-input 1" deleteLabel="bca-file__delete-label" figure="bca-file__figure" img="bca-file__img" figcaption="bca-file__figcaption 1" id="cucustomfieldvalue-article-loop-src-article-image"></span>'
        ...
        if(in_array($ext, ['png', 'gif', 'jpeg', 'jpg'])) {
            ...
            ) . '<br>' . $this->BcHtml->tag('figcaption', $this->mb_basename($value), ['class' => 'bca-file__figcaption file-name']) . '</figure>'; // カスタムフィールドではファイル名がUUIDに置換されるので表示不要
        } else {
            $fileLinkTag = '<p>' . $this->BcHtml->link(
                'ダウンロード',
                $this->saveUrl . 'cu_custom_field/' . $value,
                ['target' => '_blank', 'class' => 'bca-btn']
            ) . '</p>' . $this->BcHtml->tag('figcaption', $this->mb_basename($value), ['class' => 'bca-file__figcaption file-name']) . '</figure>'; // カスタムフィールドではファイル名がUUIDに置換されるので表示不要
            $fileLinkTag .= '<p>'. h($value).'</p>';
        }
